...
public sealed class TrickyEdgeCaseTests
{
    ...
    // ═══════════════════════════════════════════════════════
    // Category 5: CommandBuffer Edge Cases
    // ═══════════════════════════════════════════════════════

    [Fact]
    public void CommandBuffer_multiple_Sets_on_same_component_last_wins()
    {
        var world = new World();
        var entity = world.Create(new Position(1, 2));
        var buffer = new CommandBuffer(world);

        buffer.Set(entity, new Position(10, 20));
        buffer.Set(entity, new Position(30, 40));
        buffer.Set(entity, new Position(50, 60));
        buffer.Submit();
    ...
    [Fact]
    public void CommandBuffer_create_add_remove_add_different_component()
    {
        var world = new World();
        var buffer = new CommandBuffer(world);
        ...
        Assert.True(world.IsAlive(entity));
        Assert.False(world.TryGet<Position>(entity, out _));
        Assert.True(world.TryGet(entity, out Velocity v));
        Assert.Equal(new Velocity(3, 4), v);
    }

    [Fact]
    public void CommandBuffer_operations_on_destroyed_entity_should_not_crash()
    {
        var world = new World();
        var entity = world.Create(new Position(1, 2));
        ...
        var buffer = new CommandBuffer(world);
        buffer.Set(entity, new Position(99, 99));
        buffer.Add(entity, new Velocity(5, 6));

        var exception = Record.Exception(() => buffer.Submit());
    ...
    [Fact]
    public void CommandBuffer_double_Submit_is_safe()
    {
        var world = new World();
        var buffer = new CommandBuffer(world);
        var entity = buffer.Create();
    ...
    [Fact]
    public void CommandBuffer_Submit_returns_false_on_empty_buffer_after_first_submit()
    {
        var world = new World();
        var buffer = new CommandBuffer(world);
        var entity = buffer.Create();
    ...
    [Fact]
    public void CommandBuffer_creates_multiple_entities_with_different_components_independently()
    {
        var world = new World();
        var buffer = new CommandBuffer(world);

        var a = buffer.Create();
        buffer.Add(a, new Position(1, 2));
        buffer.Add(a, new Velocity(3, 4));

        var b = buffer.Create();
        buffer.Add(b, new Health(100));

        var c = buffer.Create();
        buffer.Add(c, new Position(5, 6));
        buffer.Add(c, new Health(200));

        buffer.Submit();

        Assert.True(world.TryGet(a, out Position pa) && pa == new Position(1, 2));
        Assert.True(world.TryGet(a, out Velocity va) && va == new Velocity(3, 4));
        Assert.False(world.TryGet<Health>(a, out _));

        Assert.True(world.TryGet(b, out Health hb) && hb == new Health(100));
        Assert.False(world.TryGet<Position>(b, out _));

        Assert.True(world.TryGet(c, out Position pc) && pc == new Position(5, 6));
        Assert.True(world.TryGet(c, out Health hc) && hc == new Health(200));
    }

    [Fact]
    public void CommandBuffer_mixed_create_and_existing_entity_operations_in_one_submit()
    {
        var world = new World();
        var existing = world.Create(new Position(10, 20));
        var buffer = new CommandBuffer(world);

        var created = buffer.Create();
        buffer.Add(created, new Position(30, 40));

        buffer.Set(existing, new Position(50, 60));
        buffer.Add(existing, new Velocity(5, 6));

        buffer.Submit();

        Assert.True(world.TryGet(created, out Position cp));
        Assert.Equal(new Position(30, 40), cp);

        Assert.True(world.TryGet(existing, out Position ep));
        Assert.Equal(new Position(50, 60), ep);
        Assert.True(world.TryGet(existing, out Velocity ev));
        Assert.Equal(new Velocity(5, 6), ev);
    }

    [Fact]
    public void CommandBuffer_Destroy_then_recreate_same_entity_sequence()
    {
        var world = new World();
        var entity = world.Create(new Position(1, 2));

        var destroyBuf = new CommandBuffer(world);
        destroyBuf.Destroy(entity);
        destroyBuf.Submit();
        Assert.False(world.IsAlive(entity));

        var recreateBuf = new CommandBuffer(world);
        var newEntity = recreateBuf.Create();
        recreateBuf.Add(newEntity, new Position(99, 99));
        recreateBuf.Submit();

        Assert.True(world.IsAlive(newEntity));
        Assert.True(world.TryGet(newEntity, out Position p));
