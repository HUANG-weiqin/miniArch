...
public sealed class FrameDeltaDeterminismTests
{
    ...
    // ══════════════════════════════════════════════════════════?
    // Wire size budget: OOM / DoS protection
    // ══════════════════════════════════════════════════════════?
    ...
    [Fact]
    public void Deserialize_rejects_delta_exceeding_MaxOpsPerFrame()
    {
        // Build a delta with MaxOpsPerFrame + 1 Reserve ops.
        // Each Reserve op: 1 tag + 2 varints (id=0, version=0 ≈ 3 bytes).
        var opCount = FrameDelta.MaxOpsPerFrame + 1;
        var approxSize = opCount * 3;
        var buf = new byte[approxSize];
        var pos = 0;
        for (var i = 0; i < opCount; i++)
        {
            buf[pos++] = (byte)DeltaOpKind.Reserve;
            buf[pos++] = 0; // id varint
            buf[pos++] = 0; // version varint
        }
