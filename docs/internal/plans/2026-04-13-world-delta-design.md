...
- `AddChild/RemoveChild` 不是附属命令，而是 entity 公开状态的一部分；delta 只记录最终 parent 状态，不记录中间命令历史。
...
- `AddChild/RemoveChild` 的 child entity
- `DestroyedEntities` 对应的 existing subtree 闭包

destroy 闭包必须基于当前 world 的 hierarchy 读取 existing subtree；同帧通过 `AddChild/RemoveChild/relink` 引入或移出的实体，则需要在 shadow public state 推演阶段体现到最终 destroy 结果里。
...
- 推演顺序保持与现有 runtime 一致：`create -> AddChild/RemoveChild -> add -> set -> remove -> destroy`。
- destroy 阶段必须在 shadow hierarchy 上算最终消失闭包，而不是只删显式 destroy roots。

### 折叠规则

- same-frame `create + destroy` 的临时实体不进入 delta。
- same-frame `create -> AddChild 到 doomed subtree -> 随 subtree 一起消失` 的临时实体不进入 delta。
...
- forward / backward 的 AddChild、RemoveChild、relink
...
- hierarchy 与 destroy 相关测试必须覆盖：
  ...
  - `RemoveChild` 后逃逸 destroy subtree
