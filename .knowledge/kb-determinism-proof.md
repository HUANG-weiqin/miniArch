...
> miniArch 的确定性保证覆盖 10 个维度：Entity ID 分配（LIFO free-list + shift-not-swap）、版本号（wrap-to-1）、Submit/Replay 顺序（Create→Hierarchy→Ops→Destroy）、Sort/Dedup（无 ties）、Archetype byte layout（LayoutKind.Auto 拦截）、Varint/Hash（LEB128 + SHA-256 + CRC32）、CanonicalChecksum（sort by Id + raw byte feed）、Soak 实证（259 seed × 6.4M 帧）、Placeholder E2E（两阶段 emit + scratch buffer）、**Query 迭代顺序（archetype 创建顺序 + entity 存储顺序，14 个测试守护）**。实证数据来自单 host soak（Submit vs Replay）、多 host lockstep soak（N host Replay 路径收敛）、三路 parity（Submit/Replay/Restore byte-equal）、FsCheck PBT、以及 QueryOrderingTests。已发现并修复 7 个确定性相关 bug（B1-B6 + LayoutKind.Auto）。float 运算确定性、系统执行顺序、外部输入时序等应用层维度不在 ECS 库的覆盖范围内。
