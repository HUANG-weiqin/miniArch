...
updated: 2026-06-08 (World 拆分 partial、ChunkView public API、GetFirst<T>、Query.GetChunks)
---
# User API Layering
...
- 核心组成：
  ...
  - `src/MiniArch/Ecs/Query.cs`：默认层 entity-only `foreach` 查询 + `GetChunks()` batch API
  - `src/MiniArch/Ecs/ChunkView.cs`：public readonly struct，直接包裹 Archetype，带 `AggressiveInlining`
  ...
  - `src/MiniArch/Core/ChunkViewTyped.cs`：`ChunkViews<T1..T4>()` typed chunk 视图枚举器
- 数据流：
