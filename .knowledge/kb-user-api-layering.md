...
module: MiniArch.Ecs
description: User-facing MiniArch.Ecs facade design, query foreach shape, and the boundary to MiniArch.Core
updated: 2026-04-12
---
# User API Layering
...
- 核心组成：
  ...
  - `src/MiniArch/Ecs/Query.cs`：单组件/双组件查询和零分配枚举器
...
- 如果是第一次读这个模块，先看：
  ...
  - `src/MiniArch/Ecs/Query.cs`：默认 `foreach` 的结果形状和枚举器
- 如果是修 bug，先看：
