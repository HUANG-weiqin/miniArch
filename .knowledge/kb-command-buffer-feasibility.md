...
updated: 2026-06-11 (修复 CommandStream pending batch 组件归属用链表而非前缀和，消除交错创建时组件归属错误；实现 last-wins 去重；补全 5 个回归测试)
---
