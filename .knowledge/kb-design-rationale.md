...
**提案**：给 `Query` 增加 typed entity ref loop / `IForEach<T1,T2,...>` 风格 API，由库内部用 `MemoryMarshal.GetReference` + `Unsafe.Add` 跳过每实体 `Span<T>` bounds check，例如 `query.ForEachRef<Position, Velocity, TJob>(ref job)`。

**为什么看起来对**：单独 microbench 中，内部 typed-ref loop 确实能打赢天真的 `span[row]` 写法；如果用户反复写 `pos[row] = new(... pos[row] ... vel[row] ...)`，去掉重复索引和 bounds check 会有明显收益。

**代表性验证（2026-07-09）**：补了 query-pressure 三方对比，要求 MiniArch 最优 API 必须比 Arch/Friflo 最优 API 高 **≥10%** 才有公共 API 价值。命令：`dotnet run -c Release --project tests/MiniArch.Benchmarks -- query-pressure --entity-count 100000 --duration 5 --warmup 1 --repetitions 3`。

| 场景 | Mini typed-ref | Mini span-opt | 对手最优 | 结论 |
|---|---:|---:|---:|---|
| 2 comp write (`Position += Velocity`) | 9023 ops/s | 8692 ops/s | Arch span-opt 8976 / Friflo manual-ref 8934 | typed-ref 只比对手最优高约 0.5–1.0% |
| 3 comp write (`Position += Velocity; Health++`) | 6581 ops/s | 6586 ops/s | Friflo manual-ref 6631 | typed-ref 比对手最优低约 0.8% |

HeroComing.Perf 真实场景也验证过：把同一机制试套到 `CollisionSystem.ProcessGroup` / `CollectGroups` 后，Attack 没有提升，甚至偏低。原因是 Hero pipeline 的瓶颈在 rule/effect dispatch、dictionary、filter、CommandStream 记录，而不是 query span bounds check；1000 entity 规模下 query 权重太低。

**为什么拒绝**：
1. 高质量用户写法 `ref var p = ref positions[row]; var v = velocities[row]; ...` 已经接近上限。
2. Arch/Friflo 也暴露 `Span<T>`，高级用户同样能用 `MemoryMarshal.GetReference` + `Unsafe.Add` 写到 manual-ref ceiling；这不是 miniArch 独占优势。
3. `IForEach<T1,T2,...>` API 会按 arity 膨胀，要求用户为每个组件组合写 job struct，增加公共边界和维护成本。
4. 实测没有达到 ≥10% 竞争优势，收益不足以支付 API 复杂度。

**结论**：不做 typed span wrapper、typed entity ref loop、`IForEach<T1,T2,...>` Query 家族。未来若重新提出，必须先用代表性 query-pressure benchmark 证明：MiniArch 新 API 相比 Arch/Friflo 最优写法稳定高 **≥10%**，且至少一个真实场景能吃到收益。microbench 或只打赢天真 span 写法不算证据。
