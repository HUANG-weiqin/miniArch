...
updated: 2026-07-05 (§3.9 拆分热路径类型不要用 abstract+override)
---
# 设计决策总纲
...
**正确做法**：base class 提供默认 throw 实现 + 子类用 `public new` 隐藏（完全非虚拟）。调用方持有具体子类引用（已经是现状）。等价于"手动单态化"，比 abstract+override 快 7-8%（因为同时去掉了 mode flag 分支判断）。

**判定标准**：在 ECS 热路径（每帧调用几千次的 API）上，绝对不要用 abstract+override。如果必须在 base class 内部递归调用，调 `*Core` helper 而不是公共 mutator（避免 throw stub）。
