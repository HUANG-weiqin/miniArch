...
updated: 2026-07-08 (§2.11 更新 TrackValueChanges boundary diff：Set 热路径不触达 value tracking)
---
# 设计决策总纲
...
**选择**：`World.Add<T>` 是 ensure+overwrite：组件不存在时迁移到新增组件的 archetype，组件已存在时原地覆盖值；`World.Set<T>` 是 strict set：组件不存在时抛异常；`World.Remove<T>` 在组件不存在时 no-op。

**为什么不是全 strict**：Submit/Replay 路径中，Clone 继承组件后同帧再 Add 同组件、或重复 Add 同组件，是合法且已被 soak 证明需要收敛的操作模式。历史上把 Add 视为 strict 会导致 Submit/Replay 分歧（见 `kb-code-review-findings.md` B1/B4），因此 Add 的真实语义是"确保存在并写入这个值"。

**性能影响**：`TryGetComponentIndex` 调用不是"为语义分支加的"——无论 Add 覆盖还是迁移，都必须先判断组件是否已存在。已存在分支原地写入，反而避免 archetype 迁移；不存在分支才走 edge cache + MoveEntity。
...
### 2.11 Change Tracking（TrackValueChanges + TrackTransitions）

**选择**：公共 API 分成两个不能混用的入口：

- `world.TrackValueChanges<T>()` 返回 `SharedValueChanges<T>`，读 baseline-to-current 的 `ValueChange<T>` Old/New net diff。
- `world.TrackTransitions(QueryDescription filter)` 返回 `TransitionLog`，读 Create/Destroy/Add/Remove 导致的 query membership Entered/Exited。

值变更内部用 world-owned per-component shared `ChangeTracker<T>`：同一个 world + component type 只有一个 tracker，保存 per-entity baseline；`.Changes` 扫描当前 world 并生成 diff，`.ClearAll()` 把当前值推进为新 baseline。`Set<T>` / `CommandStream.Set<T>` 不查询 registry、不写 dirty log。结构变更仍用 per-log `List<Transition>`，World 在结构操作时 dispatch 到注册 log（`IChangeQuery.OnTransition`），每个 log 独立 filter 后存入自己的 buffer。
...
| GetForWrite\<T> 写变体 API | 新增 API 面，违反 YAGNI；boundary diff 已能捕获 `GetRef<T>` / chunk span 直接写，无需新写 API |
| filtered value changes（`TrackValueChanges<T>().With<U>()`） | 会把 filter matching 塞回读端或要求 per-filter baseline，当前选择把值 diff 保持为 per-component global baseline，过滤放到消费端 |
| multi-capture value changes | 需要 union/boxing 或组合 tracker，破坏 `ReadOnlySpan<ValueChange<T>>` 的零拷贝单类型模型；需要多个组件就建多个 independent shared logs |
| per-segment 列版本 | HP 类组件 archetype 是 flat 模式（实体少），per-archetype 已是最细粒度；segment 级仅对 >2MB 大 archetype 有意义，那些组件（Position 批量写）本不需要 per-change 追踪 |
| 复合 query 的 per-component Added/Removed 日志 | 无法处理 With/Without：Add\<Dead> 让 With\<HP>.Without\<Dead> 的成员退出，但 per-HP 日志看不到。old→new signature 一条原语覆盖所有 query 形态 |
| 独立 filter 机制（非复用 QueryDescription） | 违反概念唯一——QueryDescription 已是唯一筛选抽象，QueryCache.Matches 已是唯一 archetype 匹配原语；TransitionLog 复用两者，零新概念 |
| World 持有共享 transition log（所有 cursor 读同一 log） | 需要 ClearTransitionLog 全局协调（所有 cursor drain 完才能清）；log 存全量条目（含不匹配的）；Cursor 需 min-cursor 追踪才能 trim |

**取舍**：值变更是 world-shared per-component boundary diff，结构变更是 per-filter membership log。两套机制按基数各取最优，不再统一在一个 `ChangeQuery` builder 下；这让 filter+value、多 capture 等无效组合在 API 形状上不可表达。
