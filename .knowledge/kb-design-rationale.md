...
updated: 2026-07-06 (§2.11 新增 ChangeTracking 子系统决策 + fluent filter 替代方案行, §3.10 push-event 误判, §3.11 防御性检查)
---
# 设计决策总纲
...
### 2.11 Change Tracking（Track\<T> + ModifiedChunks + Transitions）

**选择**：`world.Track<T>()` 返回有状态游标 `ChangeQuery<T>`，提供 `ModifiedChunks()`（值写入脏 chunk）和 `Transitions()`（有序 Entered/Exited 成员转移）。值变更用 per-(archetype,column) long 版本号在 3 个 Archetype 写入 chokepoint bump；结构变更用 per-ChangeQuery 的 `List<Transition>`，World 在结构操作时 dispatch 到注册的 query（`IChangeQuery.OnTransition`），每个 query 独立 filter 后存入自己的 log。
...
| GetForWrite\<T> 写变体 API | 新增 API 面，违反 YAGNI；Get/Set 已是天然读写区分（Get=read 契约），想追踪走 Set 即可 |
...
| 独立 filter 机制（非复用 QueryDescription） | 违反概念唯一——QueryDescription 已是唯一筛选抽象，QueryCache.Matches 已是唯一 archetype 匹配原语；ChangeQuery 复用两者，零新概念 |
...
**取舍**：值变更用轮询（chunk 版本，海量友好、cache 友好），结构变更用记录（退出会消失，必须当场记；稀疏所以记录成本低）。两套机制按基数各取最优，统一在 ChangeQuery 游标下。
