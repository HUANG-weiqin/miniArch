...
**选择**：`world.Track<T>()` 返回有状态游标 `ChangeQuery<T>`，提供 `ModifiedChunks()`（值写入脏 chunk）和 `Transitions()`（有序 Entered/Exited 成员转移）。值变更用 per-(archetype,column) long 版本号在 3 个 Archetype 写入 chokepoint bump；结构变更用 append-only old→new signature transition log。
