...
updated: 2026-07-03 (补 trace 优化：Create 单线程无锁化、Destroy 按需 hierarchy unavailable lookup、并发 reserve 契约明确)
---
...
- **FrameDelta.Merge** → 见本章 "Merge + id 回收" 段
...
`FrameDelta.Merge(FrameDelta a, FrameDelta b)` 是 15 行 `Array.Copy` 拼接，不做任何语义折叠：
...
代码位置：`src/MiniArch/Core/FrameDelta.cs:485-500`。

**关键约束：** `ArgumentNullException`；不检查版本兼容性；不检查 buffer overflow（帧 delta 远小于 2GB）；结果 buffer 新分配（LOH）。

**历史：** 旧实现（2026-06-14 前）是 200 行折叠状态机，遇到跨帧 entity id 回收时产生 bug——折叠丢失时序依赖。当前 `Merge` 不分析语义，只 `Array.Copy` 两个 buffer——时序完整保留。

**回归测试：** `tests/MiniArch.Tests/Core/FrameDeltaDeterminismTests.cs`：`*_merged_delta_round_trip_*` 系列。

**坑点：** Merge 不做去重；不做排序（顺序 = a 的 op 全部在前）；Merge 后的 delta 不能再 `Deserialize`（buffer 本身就是 wire format）。
