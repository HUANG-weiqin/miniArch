...
|---------|---------|---------|------|--------------|---------|-------|
| **HeroComing.Perf** | `tools/perf/HeroComing.Perf` | 30s 固定时长 movement+attack 全帧吞吐 | rounds/s | Movement 2052.7 / Attack 1246.8 | ✅ **AGENTS.md §5 强制** | `kb-hero-pipeline-regression.md` |
...
| **DestroyMany.Perf** | `tools/perf/DestroyMany.Perf` | `DestroyMany` / `Destroy(query)` vs guarded `for Destroy`，只计销毁阶段，内置 checksum/diff 正确性验证 | speedup / us/iter | 2026-07-10：full dense 2.85×；query 3.63×；full cascade 1.68×；partial cascade 2.91× | ❌ API 专项证明 | `kb-core-ecs.md` |
