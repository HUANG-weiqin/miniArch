...
| MiniArch.Core Change Tracking | `kb-change-tracking.md`（TrackValueChanges/SharedValueChanges 值变更 + TrackTransitions/TransitionLog 结构变更；为什么不是 push event） |
...
**反应式 / 变更追踪**：
- **变更追踪（渲染层反应式）** → `kb-change-tracking.md`（TrackValueChanges / TrackTransitions；为什么不是 push event 见 `kb-design-rationale.md` §3.10）
