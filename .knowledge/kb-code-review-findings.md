...
- **结论**：7 个递增点全部必要且充分。Latent note: `ConvertToChunked` 自身不递增，但 production 路径中只被 `EnsureCapacity` → `AllocateRows` 调用，后续 AllocateRows chunked path 必递增。仅 `ForceChunkedForTesting` → 立即 `GetEntities` 无中间变更时可能命中空缓存（无现有测试触发）
- **验证**：`rg "_flatEntitiesGeneration\+\+"` 确认 7 个递增点；`AssertFlatCacheConsistent` DEBUG 断言守护缓存一致性
