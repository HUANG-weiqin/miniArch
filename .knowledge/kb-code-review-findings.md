...
#### CS9. `CommandStream` 拆分为 base + sealed 子类时用 abstract+override 会 regression ~10%（已修）
...
- **修复**：base class 的 9 个 mutator 改为 `public` 非虚拟 + 默认 throw `NotSupportedException`；子类用 `public new` 隐藏（完全非虚拟，JIT direct call + inline）。修复后比原版还快 ~8%（因为同时去掉了 `_parallelMode` 分支判断）。
