...
- 核心组成：
  ...
  - `Archetype.cs`：全部存储的持有者（`_data` `_entities` `_columnByteOffsets` `_elementSizes`）、实体计数、结构变化入口、迁移拷贝
  - `Chunk.cs`：readonly struct 视图，包裹 Archetype 暴露只读查询接口
  ...
  - `GenericMethodCache.cs`：泛型方法反射缓存
...
- 数据流 / 控制流：
  ...
  - `World.Create<T...>` 为 `1..16` 个组件提供固定重载；warmed 路径缓存在泛型 static cache（`CachedCreateArchetype`），O(1) 无分配
  - `World.EnsureCapacity` 负责提前扩好 entity metadata 存储
  - `World.CreateMany` 先批量准备 entity id，再一次性落入空签名 archetype
  - `Add/Remove` 先算目标签名，再通过 edge cache（`_addDestinationCache`/`_removeDestinationCache` 直索引数组）找到目标 Archetype，用 `Archetype.CopySharedComponentsFrom` 搬迁共享组件（遍历 destination 的 signature，只复制 source 也有的组件）
  - `Set` 在组件已存在时直接定位到 typed column 的 row，原地写回，不触发迁移
  - `EntityAccessor` 缓存 `(Archetype, RowIndex)`，后续 `Get<T>` / `Set<T>` / `Has<T>` 跳过 `_records` 查找和 version check，直接通过 `GetComponentIndexFast` + `GetComponentRefAt` 定位数据
  ...
  - Query 读路径使用 world 发布（volatile write）的 archetype 数组快照和 query 自身发布的 matched-archetype/chunk 数组快照
  ...
  - Archetype 按需线性增长：`EnsureCapacity` 按 double 策略扩容，每列 `CopyBlockUnaligned` 整列搬移
...
- 最重要的抽象：`World` → `Signature` → `Archetype`（Chunk 是 Archetype 的 readonly 视图）

## 入口

- 第一次读：`World.cs`（完整控制流入口）→ `Signature.cs`（archetype key 规则）→ `Archetype.cs`（存储和迁移）→ `Chunk.cs`（视图包装器）
- EntityAccessor：`EntityAccessor.cs`（ref struct，单实体多组件直读）
- 修 bug：`World.cs`（实体迁移和版本校验）→ `Query.cs`（快照刷新逻辑）
...
- Query 快照是非原子的（archetype 和 chunk 数组分开写入），安全性依赖 volatile publish + "world 无并发写"前提
...
- 本页描述的是当前实现（Archetype 扁平化），不是旧版"多 Chunk + MigrationPlan + ArchetypeEdges"架构
