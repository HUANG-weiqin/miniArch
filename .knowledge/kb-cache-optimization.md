...
```
World (5 partial files — 详见 kb-architecture-review.md §10)
...
> **注意**：这里的 Movement-Stream 1818 rounds/s 是 `SubmitAndSnapshotAsync` 路径的 2026-06-22 历史独立测量；当前 `HeroComing.Perf` 回归门禁的 `Submit` 路径 baseline 见 `kb-hero-pipeline-regression.md`（2026-07-06 为 Movement 2052.7 / Attack 1246.8）。两者是不同 harness，不要直接横向比较——详见 `kb-perf-harnesses.md`。
...
- **看迭代热路径**：`Core/QueryCache.cs` 的 `EnsureRefreshed()` / `AppendNewArchetypes()`
