...
- `MiniArch.Ecs` 的 query façade 只覆盖 1~2 组件查询
...
| 需要 `with/without/any` 组合过滤 | `MiniArch.Core` | `QueryBuilder`、`Query`、`QueryDescription` 都在这里 |
...
`world.Query<T>()` / `world.Query<T1, T2>()` 返回的就是 `MiniArch.Ecs` 默认查询对象，并且默认是只读枚举。常见闭环是：`Create` -> `foreach` 读取 -> `Set` 写回；`TryGet` 用于按 entity 直接读取或校验组件。若你要 3+ 组件查询、复杂过滤、chunk 级遍历或更底层控制，再切到 `MiniArch.Core`。
...
### 示例 1：可复用 Advanced QueryDescription `MT-Read`
...
边界可以直接这样记：`MiniArch.Ecs = Create/Add/Set/Remove/Destroy/Link/Unlink/TryGetParent/GetChildren/IsAlive/TryGet/Query<T>/Query<T1, T2>`。超过这条线的 3+ 组件查询、过滤、chunk、command buffer、snapshot 需求，再进入 `MiniArch.Core`。

#### `MiniArch.Ecs.World`

`MiniArch.Core.World` 的薄 façade。

普通层的核心常用路径可以直接记成：`Create/Add/Set/Remove/Destroy/Link/Unlink/TryGetParent/GetChildren/IsAlive/TryGet/Query<T>/Query<T1, T2>`。如果你要 3+ 组件查询、过滤查询、chunk 级访问、command buffer 或 snapshot，就切到 `MiniArch.Core`。

再强调一次：`MiniArch.Ecs` 只提供 `Query<T>()` 和 `Query<T1, T2>()` 这两条默认 query façade。`Query<T1, T2, T3>()`、`QueryBuilder`、`QueryDescription` 都属于 `MiniArch.Core`。
...
- 如果你只需要 1~2 个组件查询，优先从 `MiniArch.Ecs` 开始，而不是直接跳到 `MiniArch.Core`。
