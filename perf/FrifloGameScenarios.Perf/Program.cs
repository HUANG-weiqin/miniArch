...
public sealed class MiniBulletHell : IGameScenario
{
    ...
    [MethodImpl(MethodImplOptions.NoInlining)]public long RunIteration(){long s=0;foreach(var c in _q.Chunks){int n=c.Count;var sp=c.GetSpan<Position>();var sv=c.GetSpan<Velocity>();for(int i=0;i<n;i++)s+=sp[i].X+sv[i].VY;}return s;}
}
...
public sealed class MiniMMOZone : IGameScenario
{
    ...
    public void Warmup(int n){for(int i=0;i<n;i++)RunIteration();}public void Dispose(){}
    [MethodImpl(MethodImplOptions.NoInlining)]public long RunIteration(){long s=0;foreach(var c in _q.Chunks){var sp=c.GetSpan<Position>();var sh=c.GetSpan<Health>();for(int i=0;i<c.Count;i++)s+=sp[i].X+sh[i].Value;}return s;}
}
...
public sealed class MiniWaveSpawner : IGameScenario
{
    ...
    public void Warmup(int n){for(int i=0;i<n;i++)RunIteration();}public void Dispose(){}
    [MethodImpl(MethodImplOptions.NoInlining)]public long RunIteration(){long s=0;int sc=_r.Next(500,1500);for(int i=0;i<sc;i++){var e=_w.Create(new Position(_r.Next(1000),_r.Next(1000)),new Velocity(_r.Next(5),_r.Next(5)),new Lifetime(_r.Next(30,120)));_a.Enqueue(e);s+=e.Id;}foreach(var c in _q.Chunks){var sl=c.GetSpan<Lifetime>();for(int i=0;i<c.Count;i++)s+=sl[i].Ticks;}int d=Math.Min(_a.Count-20000,2000);for(int i=0;i<d&&_a.Count>0;i++){var e=_a.Dequeue();_w.Destroy(e);s+=e.Id;}return s;}
}
...
public sealed class MiniRPGStats : IGameScenario
{
    ...
    public void Warmup(int n){for(int i=0;i<n;i++)RunIteration();}public void Dispose(){}
    [MethodImpl(MethodImplOptions.NoInlining)]public long RunIteration(){long s=0;foreach(var c in _q.Chunks){var sp=c.GetSpan<Position>();var sv=c.GetSpan<Velocity>();var sh=c.GetSpan<Health>();var sm=c.GetSpan<Mana>();var sa=c.GetSpan<Armor>();for(int i=0;i<c.Count;i++)s+=sp[i].X+sv[i].VY+sh[i].Value+sm[i].Value+sa[i].Value;}return s;}
}
...
public sealed class MiniConditionalEffects : IGameScenario
{
    ...
    public void Warmup(int n){for(int i=0;i<n;i++)RunIteration();}public void Dispose(){}
    [MethodImpl(MethodImplOptions.NoInlining)]public long RunIteration(){long s=0;foreach(var c in _pq.Chunks){var sh=c.GetSpan<Health>();for(int i=0;i<c.Count;i++)s+=sh[i].Value;}foreach(var c in _bq.Chunks){var sh=c.GetSpan<Health>();for(int i=0;i<c.Count;i++)s+=sh[i].Value;}return s;}
}
...
public sealed class MiniAIStateMachine : IGameScenario
{
    ...
    public void Warmup(int n){for(int i=0;i<n;i++)RunIteration();}public void Dispose(){}
    [MethodImpl(MethodImplOptions.NoInlining)]public long RunIteration(){long s=0;for(int i=0;i<_es.Length/5;i++){var e=_es[_r.Next(_es.Length)];if(_w.Has<StateIdle>(e))_w.Remove<StateIdle>(e);if(_w.Has<StateMove>(e))_w.Remove<StateMove>(e);if(_w.Has<StateAttack>(e))_w.Remove<StateAttack>(e);if(_w.Has<StateDead>(e))_w.Remove<StateDead>(e);switch(_r.Next(4)){case 0:_w.Add(e,new StateIdle(0));break;case 1:_w.Add(e,new StateMove(0));break;case 2:_w.Add(e,new StateAttack(0));break;case 3:_w.Add(e,new StateDead(0));break;}s+=e.Id;}foreach(var c in _iq.Chunks){var sp=c.GetSpan<Position>();for(int i=0;i<c.Count;i++)s+=sp[i].X;}foreach(var c in _mq.Chunks){var sp=c.GetSpan<Position>();for(int i=0;i<c.Count;i++)s+=sp[i].X;}foreach(var c in _aq.Chunks){var sp=c.GetSpan<Position>();for(int i=0;i<c.Count;i++)s+=sp[i].X;}return s;}
}
...
public sealed class MiniTeamAlternation : IGameScenario
{
    ...
    public void Warmup(int n){for(int i=0;i<n;i++)RunIteration();}public void Dispose(){}
    [MethodImpl(MethodImplOptions.NoInlining)]public long RunIteration(){_t++;long s=0;var q=_t%2==1?_aq:_bq;foreach(var c in q.Chunks){var sp=c.GetSpan<Position>();var sh=c.GetSpan<Health>();for(int i=0;i<c.Count;i++)s+=sp[i].X+sh[i].Value;}return s;}
}
...
public sealed class MiniMixedLoad : IGameScenario
{
    ...
    public void Warmup(int n){for(int i=0;i<n;i++)RunIteration();}public void Dispose(){}
    [MethodImpl(MethodImplOptions.NoInlining)]public long RunIteration(){long s=0;for(int i=0;i<200;i++){var e=_w.Create(new Position(_n,_n),new Health(50+_r.Next(50)),new Damage(3+_r.Next(7)),new Team(_r.Next(4)));_a.Add(e);_n++;s+=e.Id;}int mc=Math.Min(5000,_a.Count);for(int i=0;i<mc;i++)s+=_a[_r.Next(_a.Count)].Id;foreach(var c in _hq.Chunks){var sh=c.GetSpan<Health>();var st=c.GetSpan<Team>();for(int i=0;i<c.Count;i++)s+=sh[i].Value+st[i].Value;}foreach(var c in _pq.Chunks){var sp=c.GetSpan<Position>();for(int i=0;i<c.Count;i++)s+=sp[i].X;}int dc=Math.Min(200,_a.Count-10000);for(int i=0;i<dc&&_a.Count>0;i++){var e=_a[0];_a.RemoveAt(0);_w.Destroy(e);s+=e.Id;}return s;}
}
