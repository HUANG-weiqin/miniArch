var cfg = MiniArch.LockstepSoak.LockstepSoakConfig.FromArgs(args);

if (cfg.Determinism)
{
    var runner1 = new MiniArch.LockstepSoak.LockstepSoakRunner(cfg);
    var ok1 = runner1.Run();
    var runner2 = new MiniArch.LockstepSoak.LockstepSoakRunner(cfg);
    var ok2 = runner2.Run();
...
else if (cfg.SweepCount > 0)
{
    ...
    for (var i = 0; i < cfg.SweepCount; i++)
    {
        ...
        var runner = new MiniArch.LockstepSoak.LockstepSoakRunner(seedCfg);
        var ok = runner.Run();
        if (ok)
        {
...
{
    var runner = new MiniArch.LockstepSoak.LockstepSoakRunner(cfg);
