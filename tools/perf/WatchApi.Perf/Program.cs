...
// ── Candidate data structures for set-diff microbenchmarks ───────

sealed class HashSetDiff
{
    private readonly HashSet<int> _snapshot = [];
    private readonly HashSet<int> _current = [];

    public HashSet<int> SnapshotSet => _snapshot;
    public HashSet<int> CurrentSet => _current;

    public (long Entered, long Exited, long Checksum) Diff()
    {
        long entered = 0, exited = 0, checksum = 0;

        foreach (var id in _snapshot)
        {
        ...
        foreach (var id in _current)
        {
            if (!_snapshot.Contains(id))
            {
                entered++;
                checksum += id;
            }
        ...
        return (entered, exited, checksum);
    }
}

sealed class DenseEpochDiff
{
    private readonly int[] _epochSnapshot;
    private readonly int[] _epochCurrent;
    private int _gen;
    private int _snapshotGen;

    // Stored snapshot entities for iteration during Diff
    private int[] _snapshotEntities = [];
    private int _snapshotCount;

    public DenseEpochDiff(int capacity)
    {
        _epochSnapshot = new int[capacity];
        _epochCurrent = new int[capacity];
    }

    public void Snapshot(int[] ids, int count)
    {
        _gen++;
        _snapshotGen = _gen;
        EnsureCapacity(ids, count);

        for (var i = 0; i < count; i++)
            _epochSnapshot[ids[i]] = _gen;

        if (_snapshotEntities.Length < count)
            _snapshotEntities = new int[count * 2];
        Array.Copy(ids, _snapshotEntities, count);
        _snapshotCount = count;
    }

    [MethodImpl(MethodImplOptions.NoInlining)]
    public (long Entered, long Exited, long Checksum) Diff(int[] currentIds, int count)
    {
        _gen++;
        EnsureCapacity(currentIds, count);

        for (var i = 0; i < count; i++)
            _epochCurrent[currentIds[i]] = _gen;

        long entered = 0, exited = 0, checksum = 0;

        // Exited: snapshot entities not in current (_epochCurrent[id] != current gen)
        for (var i = 0; i < _snapshotCount; i++)
        {
            var id = _snapshotEntities[i];
            if (_epochCurrent[id] != _gen)
            {
                exited++;
                checksum += id;
            }
        ...
        // Entered: current entities not in snapshot (_epochSnapshot[id] != snapshot gen)
        for (var i = 0; i < count; i++)
        {
            var id = currentIds[i];
            if (_epochSnapshot[id] != _snapshotGen)
            {
                entered++;
                checksum += id;
            }
    ...
    private void EnsureCapacity(int[] ids, int count)
    {
        if (count == 0) return;
        var max = ids[0];
        for (var i = 1; i < count; i++)
            if (ids[i] > max) max = ids[i];
        if ((uint)max >= (uint)_epochSnapshot.Length)
            throw new InvalidOperationException(
                $"DenseEpochDiff capacity ({_epochSnapshot.Length}) exceeded by id {max}. " +
                $"Use --entity-count less than capacity.");
    }

    public int SnapshotCount => _snapshotCount;
    public int EpochGen => _gen;
}

sealed class BitSetDiff
{
    private readonly ulong[] _snapshotBits;
    private readonly ulong[] _currentBits;
    private int[] _snapshotEntities = [];
    private int _snapshotCount;

    public BitSetDiff(int capacity)
    {
        var wordCount = (capacity + 63) / 64;
        _snapshotBits = new ulong[wordCount];
        _currentBits = new ulong[wordCount];
    }

    public void Snapshot(int[] ids, int count)
    {
        Array.Clear(_snapshotBits, 0, _snapshotBits.Length);

        for (var i = 0; i < count; i++)
        {
            var id = ids[i];
            _snapshotBits[id >> 6] |= 1UL << (id & 63);
        }
    ...
    public (long Entered, long Exited, long Checksum) Diff(int[] currentIds, int count)
    {
        // Clear current bits efficiently by only touching words that were set
        if (_snapshotCount > 0)
        {
            // Clear only words touched by current IDs (we'll set them again below)
            // But we don't know which words were set in the previous call.
            // For simplicity, clear all. BitArray has the same issue.
        }
        Array.Clear(_currentBits, 0, _currentBits.Length);

        for (var i = 0; i < count; i++)
        {
            var id = currentIds[i];
            _currentBits[id >> 6] |= 1UL << (id & 63);
        }

        long entered = 0, exited = 0, checksum = 0;

        // Exited: snapshot entities not in current
        for (var i = 0; i < _snapshotCount; i++)
        {
            var id = _snapshotEntities[i];
            var word = _currentBits[id >> 6];
            if ((word & (1UL << (id & 63))) == 0)
            {
                exited++;
                checksum += id;
        ...
        // Entered: current entities not in snapshot
        for (var i = 0; i < count; i++)
        {
            var id = currentIds[i];
            var word = _snapshotBits[id >> 6];
            if ((word & (1UL << (id & 63))) == 0)
            {
                entered++;
                checksum += id;
...
static class Program
{
    static void Main(string[] args)
    {
        ...
        var scenarios = new (string Name, Action<int, double, double> Run)[]
        {
            ...
            ("candidate-hashset-nochange", RunCandidateHashSetNoChange),
            ("candidate-hashset-churn", RunCandidateHashSetChurn),
            ("candidate-dense-epoch-nochange", RunCandidateDenseEpochNoChange),
            ("candidate-dense-epoch-churn", RunCandidateDenseEpochChurn),
            ("candidate-bitset-nochange", RunCandidateBitSetNoChange),
            ("candidate-bitset-churn", RunCandidateBitSetChurn),
        };
    ...
    // ═══════════════════════════════════════════════════════════════
    //  Candidate microbenchmarks (no World, pure set-diff kernel)
    // ═══════════════════════════════════════════════════════════════

    // ── Helper: generate current IDs array for churn scenarios ────

    static int[] BuildChurnIds(int entityCount, int churnCount, int iteration)
    {
        var ids = new int[entityCount];
        var offset = (iteration * churnCount) % entityCount;
        for (var i = 0; i < entityCount; i++)
            ids[i] = (i + offset) % entityCount;
        // Now swap out churnCount entries with "new" IDs beyond entityCount
        for (var t = 0; t < churnCount; t++)
        {
            var swapIdx = (offset + t) % entityCount;
            ids[swapIdx] = entityCount + t;
        }
        return ids;
    }

    // ── Candidate: hashset-nochange ───────────────────────────────

    static void RunCandidateHashSetNoChange(int entityCount, double warmupSeconds, double durationSeconds)
    {
        var snapshotIds = new int[entityCount];
        var currentIds = new int[entityCount];
        for (var i = 0; i < entityCount; i++)
        {
            snapshotIds[i] = i;
            currentIds[i] = i;
        ...
        var diff = new HashSetDiff();

        // Warmup
        WarmupTime(() =>
        {
            diff.SnapshotSet.Clear();
            diff.CurrentSet.Clear();
            foreach (var id in snapshotIds) diff.SnapshotSet.Add(id);
            foreach (var id in currentIds) diff.CurrentSet.Add(id);
            diff.Diff();
        }, warmupSeconds);
        ...
        PrintResult("candidate-hashset-nochange", entityCount, ops, elapsedMs, allocBytes, checksum);
    }

    // ── Candidate: hashset-churn ──────────────────────────────────

    static void RunCandidateHashSetChurn(int entityCount, double warmupSeconds, double durationSeconds)
    {
        var churnCount = Math.Max(1, entityCount / 100);
        var snapshotIds = new int[entityCount];
        for (var i = 0; i < entityCount; i++)
        ...
        // Pre-build ID arrays for one full cycle of the churn pattern.
        // The pattern repeats every entityCount iterations.
        var period = entityCount;
        var allCurrentIds = new int[period][];
        for (var i = 0; i < period; i++)
            allCurrentIds[i] = BuildChurnIds(entityCount, churnCount, i);
        ...
        WarmupTime(() =>
        {
            var ids = allCurrentIds[warmupIdx % period];
            warmupIdx++;
            diff.SnapshotSet.Clear();
            diff.CurrentSet.Clear();
            foreach (var id in snapshotIds) diff.SnapshotSet.Add(id);
            foreach (var id in ids) diff.CurrentSet.Add(id);
            diff.Diff();
        }, warmupSeconds);
        ...
        var (ops, elapsedMs, allocBytes, checksum) = MeasureTime(() =>
        {
            var ids = allCurrentIds[measureIdx % period];
            measureIdx++;
            diff.SnapshotSet.Clear();
            diff.CurrentSet.Clear();
            foreach (var id in snapshotIds) diff.SnapshotSet.Add(id);
        ...
        PrintResult("candidate-hashset-churn", entityCount, ops, elapsedMs, allocBytes, checksum);
    }

    // ── Candidate: dense-epoch-nochange ───────────────────────────

    static void RunCandidateDenseEpochNoChange(int entityCount, double warmupSeconds, double durationSeconds)
    {
        var snapshotIds = new int[entityCount];
        var currentIds = new int[entityCount];
        for (var i = 0; i < entityCount; i++)
        ...
        // Capacity: entityCount is the max ID used
        var diff = new DenseEpochDiff(entityCount);

        // Warmup (no checksum accumulation)
        WarmupTime(() =>
        {
            diff.Snapshot(snapshotIds, entityCount);
            diff.Diff(currentIds, entityCount);
        }, warmupSeconds);
        ...
        {
            diff.Snapshot(snapshotIds, entityCount);
            var result = diff.Diff(currentIds, entityCount);
            return result.Checksum;
        }, durationSeconds);

        PrintResult("candidate-dense-epoch-nochange", entityCount, ops, elapsedMs, allocBytes, checksum);
    }

    // ── Candidate: dense-epoch-churn ──────────────────────────────

    static void RunCandidateDenseEpochChurn(int entityCount, double warmupSeconds, double durationSeconds)
    {
        var churnCount = Math.Max(1, entityCount / 100);
        var snapshotIds = new int[entityCount];
        for (var i = 0; i < entityCount; i++)
        ...
        var diff = new DenseEpochDiff(entityCount + churnCount);
        ...
        var (ops, elapsedMs, allocBytes, checksum) = MeasureTime(() =>
        {
            ...
            measureIdx++;
            diff.Snapshot(snapshotIds, entityCount);
            var result = diff.Diff(ids, entityCount);
            return result.Checksum;
        }, durationSeconds);

        PrintResult("candidate-dense-epoch-churn", entityCount, ops, elapsedMs, allocBytes, checksum);
    }

    // ── Candidate: bitset-nochange ────────────────────────────────

    static void RunCandidateBitSetNoChange(int entityCount, double warmupSeconds, double durationSeconds)
    {
        var snapshotIds = new int[entityCount];
        var currentIds = new int[entityCount];
        for (var i = 0; i < entityCount; i++)
        ...
        var diff = new BitSetDiff(entityCount);
        ...
        PrintResult("candidate-bitset-nochange", entityCount, ops, elapsedMs, allocBytes, checksum);
    }

    // ── Candidate: bitset-churn ───────────────────────────────────

    static void RunCandidateBitSetChurn(int entityCount, double warmupSeconds, double durationSeconds)
    {
        var churnCount = Math.Max(1, entityCount / 100);
        var snapshotIds = new int[entityCount];
        for (var i = 0; i < entityCount; i++)
        ...
        var diff = new BitSetDiff(entityCount + churnCount);
        ...
        PrintResult("candidate-bitset-churn", entityCount, ops, elapsedMs, allocBytes, checksum);
    }
}
