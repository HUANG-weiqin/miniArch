...
else if (cfg.SweepCount > 0)
{
    if (cfg.Async)
    {
        // Async diversity sweep: run N consecutive seeds
        var passed = 0;
...
else
{
    if (cfg.Async)
    {
        ...
        var passed = await runner.RunAsync();
